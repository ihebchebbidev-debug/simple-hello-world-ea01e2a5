...
import { Auth, LOCALE_KEY } from '../services/api';
...
export default function SplashScreen({ navigation }) {
  ...
  return (
    <View style={[s.root, { backgroundColor: BG }]}>
      ...
      {/* Ghost bus watermark */}
      <View style={[s.busBg, { width: width * 1.0, height: width * 1.0 }]} pointerEvents="none">
        <MaterialCommunityIcons name="bus" size={width * 0.9} color={colors.primary} style={{ opacity: 0.04 }} />
      </View>
        ...
        <Animated.View style={{ opacity: logoOp, transform: [{ scale: logoScale }], marginBottom: spacing.xl }}>
          <View style={s.outerRing}>
            <View style={s.innerRing}>
              <Image source={logoMark} style={{ width: LOGO + 12, height: LOGO + 12 }} resizeMode="contain" />
            </View>
          </View>
        </Animated.View>
...
const s = StyleSheet.create({
  ...
  busBg: { position: 'absolute', bottom: -80, right: -80, overflow: 'hidden' },
  center: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.xl },

  outerRing: {
    width: LOGO + 56, height: LOGO + 56, borderRadius: (LOGO + 56) / 2,
    borderWidth: 1, borderColor: 'rgba(61,191,201,0.18)',
    alignItems: 'center', justifyContent: 'center',
    backgroundColor: 'rgba(61,191,201,0.04)',
  },
  innerRing: {
    width: LOGO + 28, height: LOGO + 28, borderRadius: (LOGO + 28) / 2,
    borderWidth: 1, borderColor: 'rgba(61,191,201,0.28)',
    alignItems: 'center', justifyContent: 'center',
  ...
  brand: { ...typography.display, fontSize: 40, color: colors.textInverse, marginBottom: spacing.xs },
  brandCompact: { fontSize: 34 },
  taglinePill: {
    backgroundColor: 'rgba(61,191,201,0.15)', borderRadius: 20,
    borderWidth: 1, borderColor: 'rgba(61,191,201,0.25)',
    paddingHorizontal: 12, paddingVertical: 4, marginBottom: spacing.md,
  },
  tagline: { fontSize: 10, fontWeight: '700', color: colors.primaryAccent, letterSpacing: 2 },
  subtitle: { ...typography.bodySm, color: colors.onBrandMuted, textAlign: 'center', maxWidth: 240 },

  barTrack: {
    height: 2, borderRadius: 1, backgroundColor: colors.onBrandHigh,
    overflow: 'hidden', marginBottom: spacing.sm,
  },
  barFill: {
    height: '100%', borderRadius: 1, backgroundColor: colors.primary,
    shadowColor: colors.primary, shadowOpacity: 0.8, shadowRadius: 4,
    shadowOffset: { width: 0, height: 0 },
  },
  loadingLabel: { ...typography.overline, color: colors.onBrandText, opacity: 0.45, textTransform: 'uppercase' },
});
